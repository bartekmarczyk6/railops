...
export function EmailPanel({
  ...
  const claimsBlock = (
    <div
      data-field="claims"
      aria-label="What the passenger wants"
      className={embedded ? "grid gap-2 border-t border-line pt-3" : "border-t border-line bg-inset/40 p-4"}
    >
  ...
  const draftBlock = showDraft ? (
    <div
      data-field="draft-response"
      aria-label="Draft response"
      className={embedded ? "grid gap-2 border-t border-line pt-3" : "grid gap-2 border-t border-line p-4"}
    >
      <h3 className="m-0 text-[11px] font-semibold tracking-wide text-ink-3 uppercase">
  ...
  if (embedded) {
    return (
      <div data-component="email-panel" data-section="email" className="grid gap-3">
        <div data-field="inbound" className="grid gap-3">
          {senderRow}
          {bodyBlock}
          {mentionsBlock}
        </div>
